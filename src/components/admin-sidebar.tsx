  ...
  return (
    <div className="w-72 bg-sidebar border-r border-sidebar-border h-[calc(100vh-64px)] p-6 sticky top-16 hidden lg:flex flex-col">
      <div className="space-y-1 mb-10">
        <h2 className="px-4 text-[10px] font-black uppercase tracking-[0.2em] text-foreground mb-4">Management</h2>
        {sidebarItems.map((item) => (
          <Link 
            key={item.href}
            to={item.href} 
            className="flex items-center justify-between p-3 rounded-2xl text-sm font-bold text-foreground hover:bg-sidebar-accent hover:text-sidebar-accent-foreground transition-all group"
      ...
      <div className="mt-auto">
        <div className="p-4 bg-sidebar-accent/50 rounded-3xl border border-sidebar-border">
          <div className="text-[10px] font-black uppercase tracking-widest text-foreground mb-2 text-center">Need Help?</div>
