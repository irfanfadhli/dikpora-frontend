              ...
              <form onSubmit={handleSubmit} className="space-y-6 mt-4">
                <div className="space-y-4">
                  <div className="grid gap-2">
                    ...
                    <Input name="name" defaultValue={editingRoom?.name} required className="rounded-xl h-12 bg-neutral-50 dark:bg-neutral-900 border-none px-4" />
                  </div>
                  <div className="grid gap-2">
                    <label className="text-[10px] font-black uppercase tracking-widest text-neutral-400">Location</label>
                    <Input name="location" defaultValue={editingRoom?.location} className="rounded-xl h-12 bg-neutral-50 dark:bg-neutral-900 border-none px-4" />
                  </div>
                  <div className="grid grid-cols-2 gap-4">
                    <div className="grid gap-2">
                      <label className="text-[10px] font-black uppercase tracking-widest text-neutral-400">Capacity</label>
                      <Input name="capacity" type="number" defaultValue={editingRoom?.capacity} className="rounded-xl h-12 bg-neutral-50 dark:bg-neutral-900 border-none px-4" />
                    </div>
                  ...
                  <div className="grid gap-2 pt-2">
                    ...
                    <div className="relative border-2 border-dashed border-neutral-100 dark:border-neutral-800 rounded-2xl p-6 text-center group hover:border-neutral-300 transition-colors">
              ...
              {isLoading ? (
                [1, 2, 3, 4].map(i => (
                  <TableRow key={i} className="animate-pulse">
                    <TableCell colSpan={6} className="h-20 px-6"><div className="h-4 bg-neutral-100 dark:bg-neutral-900 rounded-full w-full" /></TableCell>
                  </TableRow>
                ))
              ) : filteredRooms.map(room => (
                <TableRow key={room.id} className="group border-b border-neutral-100 dark:border-neutral-900 hover:bg-neutral-50/50 dark:hover:bg-neutral-900/50 transition-colors">
                  <TableCell className="px-6 py-4">
                    <div className="w-12 h-10 rounded-xl bg-neutral-100 dark:bg-neutral-900 overflow-hidden flex items-center justify-center">
                      {room.image && (
                        <img 
                          src={room.image} 
